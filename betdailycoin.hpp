/**
 * This contract interacts with the 'dailycoinutx' Dailycoin token contract to allow
...
namespace eosio {
   ...
   class [[eosio::contract("betdailycoin")]] dice : public contract {
      public:
	 ...
	 void token_transfer( name from, name to, asset quantity, string memo );

	 static constexpr name TOKEN_CONTRACT = name("dailycoinutx");

   private:

	 static constexpr symbol TOKEN_SYMBOL = symbol("XDL", 4);
	 static constexpr name NULL_NAME = name("");
	 static const uint32_t NULL_GUESS = 0x7FFFFFFF;
	 static const int MAX_BET_TO_BANKROLL_RATIO = 100; // Bets can't exceed 1% of a host's bankroll.
	 static const int MIN_TRANSFER_FRACTIONS = 100; // Minimum deposit+bet & withdrawal amount in token fractions.
	 static const uint32_t GAME_TIMEOUT_SECS = 60*60;
	 static const uint64_t ZERO_SOURCE = 0x6c8fc18b8e9f8e20; // 64-bit commitment key of a 0x00..000 32-byte sha256 hash source
	 inline static const asset ZERO_TOKENS = asset(0, TOKEN_SYMBOL);
	 inline static const asset TOKEN_FRACTION = asset(1, TOKEN_SYMBOL);
	 inline static const asset MIN_BALANCE = asset(10000000, TOKEN_SYMBOL);
	 ...
	 void do_bet( name player, asset quantity, uint32_t guess, uint8_t bits );
	 ...
	 static uint64_t get_mult(int64_t bits) { uint64_t m = 2; for (int i = 1; i < bits ; ++i) m <<= 1; return m - 1; }
   };
